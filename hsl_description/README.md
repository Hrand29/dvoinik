...
Это минимум, нужный самому двойнику. В репозитории есть ещё два пакета,
которые двойнику не требуются, но нужны для задач HSL26 — `robot_slam`
(SLAM) и `robot_b_detector` (детекция второго робота); их сборка и запуск
...
| `gui` | `true` | 3D-окно самого Gazebo. `gui:=false` — без него: дублирует RViz и заметно увеличивает задержку потока лидара на загруженной машине |
| `rviz` | `true` | `rviz:=false` — не поднимать RViz |
...
| `lidar_noise_stddev` | `0.005` | гауссов шум дальности лидара, м (1σ). ИЗМЕРЕН по реальной записи квалификации (фит плоскости стены на 1/2/3 м). `lidar_noise_stddev:=0.0` — ground truth, для сравнения |
...
### ⚠️ Одометрия и шум сенсоров — как на реальном роботе

`/odom` больше НЕ ground truth: `odometry_source=0` (счисление по колёсам),
а `scripts/odometry_bridge.py` перезаписывает курс гироскопом — ровно как
`kobuki_node` с `use_imu_heading: true`. Поэтому **x,y дрейфуют от
проскальзывания, а курс нет**, и `publish_odom_tf` у плагина отключён: TF
`odom->base_link` публикует мост, чтобы топик и TF не разошлись.

Шум сенсоров включён по умолчанию (лидар и Livox IMU — по измерениям с
реальной записи, гироскоп kobuki — оценка). Курс уходит на 0.3–1.5
град/мин, смещение разыгрывается заново при каждом запуске.

В RViz ставьте **Fixed Frame = `map`** при работе со SLAM: в кадре `odom`
робот будет «ехать сквозь стену», а карта уползать — это нормальное
поведение цепочки `map->odom->base_link`, а не ошибка.
