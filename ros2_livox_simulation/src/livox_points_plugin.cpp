...
namespace gazebo
{
    ...
    void LivoxPointsPlugin::OnNewLaserScans()
    {
        ...
        if (rayShape)
        {
            ...
            // Патч: исходник этого форка писал в CustomMsg старой схемы
            // (плоские параллельные массивы xs[]/ys[]/zs[]/frame_id[]/
            // publish_time_ns/timebase_ns) — так CustomMsg.msg выглядел в
            // более старой версии livox_ros_driver2. У реального driver'а
            // организаторов (hackaton/hsl25-master) схема другая: header
            // (стандартный std_msgs/Header) + timebase + point_num + lidar_id
            // + rsvd + CustomPoint[] points. Ниже — под реальную схему.
            const std::string frame = raySensor->Name();
            pp_livox.header.stamp = stamp;
            pp_livox.header.frame_id = frame;
            pp_livox.timebase = now_ns;
            pp_livox.lidar_id = 0;
            ...
            for (auto &pair : points_pair)
            {
                ...
                auto rotate_info = pair.second;
                {
                    ...
                    // offset_time: наносекунды от начала обработки этого
                    // накопительного окна (используется Livox для синхронизации)
                    ...
                    p.offset_time = static_cast<uint32_t>(elapsed_time.count());
                    p.x = static_cast<float>(point.X());
                    p.y = static_cast<float>(point.Y());
                    p.z = static_cast<float>(point.Z());
                    p.reflectivity = static_cast<uint8_t>(
            ...
            // point_num уже выставлен в цикле выше
                                           ...
                                           boost::shared_ptr<physics::LivoxOdeMultiRayShape> &ray_shape)
    {
        ...
        // ⚠️ Патч: currStartIndex объявлен в .h (нерепиты-режим "sliding window"
        // по CSV), но нигде не использовался — index был чистой функцией от
        // r/stride/maxPointSize, поэтому КАЖДЫЙ вызов выбирал ОДНИ И ТЕ ЖЕ
        // строки CSV. SetPoints() ниже реально перевызывается каждый кадр
        // (сам механизм рабочий), просто направления лучей не менялись.
        // Итог: облако точек не обновлялось — было видно живым тестом
        // (при каждом запуске точки в одних и тех же местах). Добавили
        // накопительное смещение через currStartIndex.
        for (int64_t r = 0; r < ray_count; ++r)
        {
            int64_t index = (r * stride + currStartIndex) % maxPointSize;
            auto &rotate_info = aviaInfos[static_cast<size_t>(index)];
        ...
        currStartIndex = (currStartIndex + ray_count) % maxPointSize;
    }
